...
instrument, not the model. Every table in `reports/` is recomputed from the raw
per-run JSON by independent implementations in `verify/`, and CI fails the build
